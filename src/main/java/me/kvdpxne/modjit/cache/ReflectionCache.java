 ...
 * <p>
 * This class provides a thread-safe caching mechanism using {@link java.util.concurrent.ConcurrentHashMap} for storing
 * weak references to cached values. It utilizes a {@link java.lang.ref.ReferenceQueue} to track when cached values are
 * garbage collected and automatically removes the corresponding entries from the cache to prevent memory leaks.
 * </p>
 * <p>
 * Subclasses must implement the logic for computing the cached values by overriding the
 * {@link #getOrCompute(java.lang.Object, java.util.function.Supplier)} method or using it as provided. This base class
 * handles the concurrency, weak referencing, and cleanup logic.
 * </p>
 *
...
public abstract class ReflectionCache<K, V> {
  ...
  /**
   * A map of locks used to synchronize access to individual cache entries identified by their keys. This prevents race
   * conditions during the computation and storage of new values for the same key.
   */
  private final ConcurrentMap<K, Object> locks = new ConcurrentHashMap<>();

  /**
   * The main cache storage. It maps keys to {@link java.lang.ref.Reference} objects (specifically
   * {@link me.kvdpxne.modjit.util.WeakReferenceWithKey}) which hold the cached values. Using weak references allows the
   * garbage collector to reclaim the values when they are no longer strongly reachable elsewhere, preventing memory
   * leaks.
   */
  private final ConcurrentMap<K, Reference<V>> cache = new ConcurrentHashMap<>();

  /**
   * The reference queue associated with this cache. When a cached value is garbage collected, its corresponding
   * {@link me.kvdpxne.modjit.util.WeakReferenceWithKey} is enqueued here. The {@link #cleanup()} method processes this
   * queue to remove stale entries from the {@link #cache}.
   */
  ...
  /**
   * Retrieves a value from the cache associated with the given key. If the value is not present or has been garbage
   * collected, it computes the value using the provided {@code compute} supplier, stores it in the cache, and returns
   * the new value.
   * <p>
   * This method is thread-safe. It uses a per-key lock to ensure that only one thread computes the value for a specific
   * key concurrently.
   * </p>
   *
   * @param key The key whose associated value is to be returned or computed. Must not be {@code null}.
   * @param compute A {@link java.util.function.Supplier} function that computes the value if it is not found in the
   *   cache. Must not be {@code null}.
   * @return The cached value associated with the key, or the newly computed value if it was not present or had been
   *   garbage collected.
   * @throws java.lang.NullPointerException if {@code key} or {@code compute} is {@code null}.
   */
